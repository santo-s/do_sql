...
// Carga servicios
function SL4AddJavascript(jsname) {
...
function SL4AddLink(linkname) {
	var th = document.getElementsByTagName('head')[0];
	var s = document.createElement('link');
	s.setAttribute('type','text/css');
	s.setAttribute('rel','stylesheet');
	s.setAttribute('href',linkname);
	th.appendChild(s);
} 

if (typeof(tpyServicios) == 'undefined') {
	if (typeof(tpyRelPath) != 'undefined') { 
   		tpyAddJavascript(tpyRelPath+"/js/pt/servicios.js");
	}
}

function GetRadioValue(control){
	//var control = document.forms['form_name_or_id'].radio_grp_name;
	var radioValue = null;
	for(i=0; i < control.length; i++){
	    if (control[i].checked == true) {
			radioValue = control[i].value;
		}
	}
	return radioValue;
}

function SetRadioValue(control, radioValue){
	//var control = document.forms['form_name_or_id'].radio_grp_name;
	for(i=0; i < control.length; i++){
	    if (control[i].value == radioValue) {
			control[i].checked = true;
		}
	}
}

function DisableRadioValue(control, n, on_off){
	//var control = document.forms['form_name_or_id'].radio_grp_name;
	if (n == 0) {
		for(i=0; i < control.length; i++){
		    control[i].disabled = on_off
		}
	}
}

function ReadOnlyRadio(control){
	//var control = document.forms['form_name_or_id'].radio_grp_name;
	for(i=0; i < control.length; i++){
		if (!control[i].checked) control[i].disabled = true;
	}
}
...
function numOper(Oper, value1, value2) {
	var group = getLocaleInfo("groupSeparator");
	// elimina los separadoresde miles
	if ( typeof(value1) == "string" ) value1 = value1.replace(group , "");
	if ( typeof(value2) == "string" ) value2 = value2.replace(group , "");
	var dec = getLocaleInfo("decimalSeparator");
	// Cambia los separadores decimales a "."
	if ( typeof(value1) == "string" ) value1 = value1.replace(dec , ".");
	if ( typeof(value2) == "string" ) value2 = value2.replace(dec , ".");
	//
	//alert(group + ' ' + dec);
	//alert(value1+' '+value2);
	if ( value1 == null || value1 == "" ) value1 = '0';
	if ( value2 == null || value2 == "" ) value2 = '0';
	if ( typeof(value1) == "string" ) value1 = parseFloat(value1.replace("," , '.'));
	if ( typeof(value2) == "string" ) value2 = parseFloat(value2.replace("," , '.'));
	//alert(value1+' '+value2);
	if (Oper == sumOper) return value1 + value2;
	if (Oper == mulOper) return value1 * value2;
	if (Oper == divOper && value2 != 0) return value1 / value2;
	if (Oper == resOper) return value1 - value2;
	return 0;
}

function localeNumber(value1) {
	//var group = getLocaleInfo("groupSeparator");
	var dec = getLocaleInfo("decimalSeparator");
	//value1 = value1.replace(group , "#");
	//value2 = value2.replace(group , "");
	// Cambia los separadores decimales a "."
	value1 = value1+'';
	value1 = value1.replace(".", dec);
	//value1 = value1.replace("#", group);
	return value1;
}

function CCRemoveParam( url, parameter ) {
	//alert('Remove '+parameter+'   '+ url);
